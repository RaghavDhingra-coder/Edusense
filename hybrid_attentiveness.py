...
class HybridAttentivenessAnalyzer:
    ...
    def __init__(self, 
        ...
        logger.info("=" * 60)
        logger.info("🔧 Initializing Hybrid Attentiveness Analyzer")
        logger.info("=" * 60)
    ...
    def _calculate_pose_confidence(self, face_landmarks, yaw: float, pitch: float, roll: float) -> float:
        ...
        # Landmark spread confidence
        xs = [lm.x for lm in face_landmarks.landmark[:10]]
        ys = [lm.y for lm in face_landmarks.landmark[:10]]
